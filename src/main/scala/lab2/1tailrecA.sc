...
def indexSwap[A](a: Int, b: Int, list: List[A]): List[A] = {
  ...
  bSwap match {
    ...
    case _ =>
      @tailrec
      def swapRecursive(index: Int, valueOfA: A, valueOfB: A, listTail: List[A], result: List[A]): List[A] = {
        valueOfB match {
          case -1 => listTail
          case _ =>
            println(result)
            listTail match {
              ...
              case head :: tail =>
                index match {
                  case `aSwap` =>
                    swapRecursive(index + 1, head, valueOfB, tail, valueOfB :: result)
                  case `bSwap` =>
                    swapRecursive(index + 1, valueOfB, valueOfB, tail, valueOfA :: result)
                  case _ =>
                    swapRecursive(index + 1, valueOfA, valueOfB, tail, head :: result)
                }
            }
        }
      }
      ...
      pos match {
        case firstElement :: secondElement :: tl =>
          ...
          swapRecursive(0, valueA, valueB, list, Nil)
        case _ =>
          list
      }
  }
